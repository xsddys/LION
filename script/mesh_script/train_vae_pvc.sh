...
if [ ! -d "$DATA_DIR/pvc" ]; then
    echo "错误：数据类别目录 $DATA_DIR/pvc 不存在！"
    exit 1
fi
...
DATE=$(date +%m%d)
EXP_NAME="pvc_vae"
EXP_PATH="$OUTPUT_DIR/$DATE/pvc/${EXP_NAME}"
mkdir -p "$EXP_PATH"

DATA=" ddpm.input_dim 3 data.cates pvc data.dataset_type mesh data.data_dir $DATA_DIR data.normalize_global True data.recenter_per_shape False data.normalize_per_shape False"
NGPU=$1 # 
num_node=1
BS=16
total_bs=$(( $NGPU * $BS ))
...
kl=0.5 # 0.5 ,0.1, 0.0005
lr=1e-3
latent=1
skip_weight=0.01 # 0.01
sigma_offset=6.0
loss='l1_sum'

# 明确指定实验根目录
$ENT --exp_root $OUTPUT_DIR ddpm.num_steps 1 ddpm.ema 0 \
    exp_name ${DATE}/pvc/${EXP_NAME} \
    log_dir ${EXP_PATH} \
    save_dir ${EXP_PATH} \
    log_name ${EXP_PATH} \
