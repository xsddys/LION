...
if [ ! -d "$DATA_DIR/cow" ]; then
    echo "错误：数据类别目录 $DATA_DIR/cow 不存在！"
    exit 1
fi
...
DATA=" ddpm.input_dim 3 data.cates cow data.dataset_type mesh data.data_dir $DATA_DIR data.normalize_global True data.recenter_per_shape False data.normalize_per_shape False"
NGPU=$1 # 
num_node=1
BS=16
total_bs=$(( $NGPU * $BS ))
...
echo "使用数据加载线程数量: $NUM_WORKERS (降低CPU使用)"
