...
if [ ! -d "$DATA_DIR/intra" ]; then
    echo "错误：数据类别目录 $DATA_DIR/intra 不存在！"
    exit 1
fi
...
DATE=$(date +%m%d)
EXP_NAME="intra_vae"
EXP_PATH="$OUTPUT_DIR/$DATE/intra/${EXP_NAME}"
mkdir -p "$EXP_PATH"

DATA=" ddpm.input_dim 3 data.cates intra data.dataset_type mesh data.data_dir $DATA_DIR data.normalize_global True data.recenter_per_shape False data.normalize_per_shape False"
NGPU=$1 # 
num_node=1
BS=16
total_bs=$(( $NGPU * $BS ))
