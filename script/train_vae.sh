...
DATA=" ddpm.input_dim 3 data.cates car data.dataset_type pointflow data.data_dir $DATA_DIR data.normalize_global True"
NGPU=$1 # 
num_node=1
BS=32 
total_bs=$(( $NGPU * $BS ))
...
EXP_PATH="$OUTPUT_DIR/$DATE/car/${EXP_NAME}"
mkdir -p "$EXP_PATH"
...
$CMD \
    ...
    trainer.epochs 5000 data.random_subsample 1 \
